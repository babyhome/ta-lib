package com.brz.talib.core.meta;

import com.brz.talib.core.CoreAnnotated;
import com.brz.talib.core.MAType;
import com.brz.talib.core.MInteger;
import com.brz.talib.core.meta.annotation.*;
...
//               System.out.println("lookback = " + m.getName());
    ...
    static private Map<String, CoreMetaData> getTaFuncMetaInfoMap() {
        ...
        for (Method funcMethod : ms) {
            ...
            if (funcMethod.getReturnType().equals(RetCode.class)) {
                String lookbakName = fn + LOOKBACK_SUFFIX;
                Method lookbackMethod = lookbackMap.get(lookbakName);
    ...
    static private Map<String, Set<CoreMetaData>> getTaGrpMetaInfoMap() {
        ...
        Map<String, Set<CoreMetaData>> result = new TreeMap<>();
    ...
    private Annotation getParameterInfo(final int paramIndex, Class<? extends Object> paramAnnotaion, Class<? extends Object> paramExtraAnnotation) {
        if (paramIndex < 0) throw new IllegalArgumentException(INDEX_OUT_OF_BOUNDS);
        int i = 0;
        for (Annotation[] annArray : function.getParameterAnnotations()) {
            for (Annotation ann : annArray) {
                if ((ann.annotationType() == paramAnnotaion) && (paramIndex == i++)) {
    ...
    public IntegerList getOptInputIntegerList(final int paramIndex) throws IllegalArgumentException {
        return (IntegerList) getParameterInfo(paramIndex, OptInputParameterInfo.class, IntegerRange.class);
    }
    ...
    public void setOptInputParamInteger(final int paramIndex, final String string) throws IllegalArgumentException {
        try {
            Integer v = NumberUtils.parseNumber(string, Integer.class);
    ...
    public void setOptInputParamReal(final int paramIndex, final String string) throws IllegalArgumentException {
        try {
            Double v = NumberUtils.parseNumber(string, Double.class);
    ...
    public void callFunc(final int startIndex, final int endIndex, MInteger outBegIdx, MInteger outNbElement) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException {
        ...
        for (Object item : callInputParams) {
            if (PriceInputParamter.class.isAssignableFrom(item.getClass())) {
